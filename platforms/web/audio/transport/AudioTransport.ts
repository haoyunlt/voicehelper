...
export class WebRTCTransport extends AudioTransport {
    ...
    async sendAudioFrame(frame: AudioFrame): Promise<void> {
        ...
        if (frame.audio instanceof ArrayBuffer) {
            ...
            // Float32Array或其他TypedArray
            const typedArray = frame.audio as Float32Array;
            buffer = (typedArray.buffer as ArrayBuffer).slice(typedArray.byteOffset, typedArray.byteOffset + typedArray.byteLength);
        }
        this.dataChannel.send(buffer);
        
        this.updateStats({
            bytesSent: this.stats.bytesSent + buffer.byteLength,
            messagesSent: this.stats.messagesSent + 1
        });
    }
